...
Процедура ЗаписатьОбъектыКорня(ДополнительныеДанные)
	...
	Для Каждого ОбъектКонфигурации Из ДополнительныеДанные.ОбъектыКонфигурации Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ОбъектКонфигурации);
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ИсходнаяТаблица.ИмяТипаИсходное + ИсходнаяТаблица.Имя + ИсходнаяТаблица.ИД КАК СТРОКА(500)) КАК ИДЗаписи,
	|	ИсходнаяТаблица.ИмяТипаИсходное,
	|	ИсходнаяТаблица.Имя,
	|	ИсходнаяТаблица.Синоним,
	|	ИсходнаяТаблица.ИД,
	|	ОписаниеСтруктурыКонфигурации.Ссылка
	|ПОМЕСТИТЬ ЗагружаемыеЭлементы
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗагружаемыеЭлементы.ИДЗаписи КАК Ссылка,
	|	ЗагружаемыеЭлементы.Ссылка КАК ЭлементСтруктуры,
	|	ЗагружаемыеЭлементы.Ссылка.Родитель как ЭлементСтруктурыРодитель,
	|	ЗагружаемыеЭлементы.ИД,
	|	ЗагружаемыеЭлементы.Имя,
	|	ЗагружаемыеЭлементы.Синоним КАК Наименование
	|ПОМЕСТИТЬ КакДолжноБыть
	|ИЗ
	|	ЗагружаемыеЭлементы КАК ЗагружаемыеЭлементы";
	Запрос.Выполнить();
...
Процедура ОбработатьСтрокуДействий(ОбрабатываемаяСтрока, ТаблицаДействий)
	...
	Если ОбрабатываемаяСтрока.Действие = "Обновить" Тогда
		...
		РедактируемыйОбъект.Родитель = ОпределитьРодителя(ОбрабатываемаяСтрока.ЭлементСтруктурыРодителя, ТаблицаДействий);
...
функция ОпределитьРодителя(ЭлементСтруктурыРодителя, ТаблицаДействий)
	
	Родитель = Неопределено;
	
	Если ЗначениеЗаполнено(ЭлементСтруктурыРодителя) Тогда 
		СтрокиРодителя = ТаблицаДействий.НайтиСтроки(Новый Структура("ЭлементСтруктуры",ЭлементСтруктурыРодителя));
